...
- Suggests optimal settings for the laboratory
...
def configure_daemon_json() -> None:
    """Provide guidance for daemon.json configuration."""
    ...
    print("Recommended daemon.json settings for the laboratory:")
    print()
    ...
    # Location guidance
    if platform.system() == 'Windows':
        print(f"{Colours.CYAN}Location on Windows:{Colours.ENDC}")
        print("  Docker Desktop Settings > Docker Engine")
        print("  Or edit: %USERPROFILE%\\.docker\\daemon.json")
    else:
        print(f"{Colours.CYAN}Location on Linux:{Colours.ENDC}")
        print("  /etc/docker/daemon.json")
    
    print()
    print_info("After editing daemon.json, restart Docker Desktop")
...
def troubleshoot_common_issues() -> None:
    ...
    issues = [
        ...
        },
        {
            ...
            "solutions": [
                "Remove existing network: docker network rm <name>",
        ...
        },
        {
            ...
            "symptoms": ["Container exits with code 1", "exited (1)"],
            "solutions": [
                "Check logs: docker logs <container_name>",
                "Verify all required files exist in docker/ directory",
                "Ensure Python scripts have valid syntax"
            ]
        },
        {
            "problem": "WSL2 integration not working",
            "symptoms": ["Cannot access Docker from WSL", "docker: command not found in WSL"],
            "solutions": [
        ...
        },
        {
            ...
            "symptoms": ["Containers slow to start", "High CPU usage"],
            "solutions": [
                "Ensure files are in WSL filesystem, not Windows mounts",
                "Increase Docker memory allocation",
                "Disable antivirus scanning for Docker directories",
...
def main() -> int:
    ...
    parser.add_argument(
        '--daemon', 
        action='store_true',
        help='Show daemon.json recommendations'
    )
    parser.add_argument(
        '--all', '-a',
        action='store_true',
    ...
    if not any([args.diagnose, args.config, args.recommend, 
                args.troubleshoot, args.daemon, args.all]):
        args.all = True
