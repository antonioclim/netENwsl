#!/usr/bin/env python3
"""ex_14_02_verification_harness.py — Verification harness for the team project.

Computer Networks — Week 14 (WSL Environment)
ASE Bucharest, CSIE | by ing. dr. Antonio Clim

Objectives:
- Design verification strategies for network services
- Implement automated connectivity and service checks
- Analyse system behaviour through structured testing

Level: Intermediate
Estimated time: 25 minutes

Pair Programming Notes:
- Driver: Write configuration, run tests
- Navigator: Verify expected outcomes, document results
- Swap after: Configuration complete, then after first test run

Usage:
  python3 ex_14_02_verification_harness.py --config project_config.json
  python3 ex_14_02_verification_harness.py --config project_config.json --out report.json
"""
...
# PREDICTION_PROMPT
# ═══════════════════════════════════════════════════════════════════════════════
def prompt_prediction(total_checks: int) -> None:
    """Ask student to predict verification results before running."""
    print(f"\n💭 PREDICTION: How many of the {total_checks} checks do you expect to pass?")
    try:
        prediction = input("   Your prediction: ").strip()
        if prediction:
            print(f"   You predicted: {prediction}/{total_checks}. Let's verify!\n")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--predict", "-p", action="store_true", 
                        help="Enable prediction prompt before verification")
...
def main() -> int:
    ...
    # Count total checks for prediction
    total_checks = sum(
        len(target.get("checks", []))
        for target in config.get("targets", [])
    )
    
    if args.predict:
        prompt_prediction(total_checks)
