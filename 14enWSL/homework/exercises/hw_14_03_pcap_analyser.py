...
complete network traffic reports.
...
def prompt_prediction() -> None:
    """Ask student to predict packet analysis results."""
    print("\n💭 PREDICTION: What percentage of packets in a typical capture are TCP vs UDP?")
    print("   Think about common protocols: HTTP (TCP), DNS (UDP), SSH (TCP), etc.")
    try:
        prediction = input("   Your estimate (e.g., '80% TCP'): ").strip()
        if prediction:
            print(f"   Let's analyse and find out!\n")
