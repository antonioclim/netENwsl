...
def prompt_prediction() -> None:
    """Ask student to predict server behaviour."""
    print("\n💭 PREDICTION: What happens if the client sends a malformed header?")
    print("   Options: a) Server crashes  b) Server returns error  c) Server ignores")
    try:
        prediction = input("   Your answer (a/b/c): ").strip()
        if prediction:
            print(f"   Let's implement and find out!\n")
