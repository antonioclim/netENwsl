...
We encourage completing these assignments in pairs using the Driver/Navigator pattern:

| Assignment | Suggested Swap Points |
|------------|----------------------|
| HW 11.01 | After health checks, after weighted RR, after stats endpoint |
| HW 11.02 | After UDP server, after cache logic, after upstream forwarding |

See `docs/pair_programming_guide.md` for detailed protocol.

### Prediction Checkpoints

Before running your code, answer these questions:

**HW 11.01:**
- 💭 How many health check failures trigger backend removal?
- 💭 With weights 3:2:1, what distribution do you expect?
- 💭 What happens if all backends fail?

**HW 11.02:**
- 💭 Will the second query for the same domain be faster?
- 💭 What happens when cached TTL reaches zero?
- 💭 What RCODE for a non-existent domain?
...
- Handle exceptions gracefully (don't let the programme crash)
...
For assessed submissions you will also receive a time-limited challenge JSON from staff.

You must submit:
- `artifacts/anti_ai/week11_challenge.json` (provided by staff)
- `pcap/week11_capture.pcap` (captured by you while running tokenised requests and a DNS query)
