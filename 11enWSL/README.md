...
1. **File → Save As**
2. Go to: `D:\NETWORKING\WEEK11\pcap\`
...
- **Concept Analogies**: See `docs/concept_analogies.md` for everyday analogies that make technical concepts intuitive (CPA method)
...
- **Prediction Prompts**: Each exercise includes "What do you expect?" moments before execution
...
## Anti-AI submission artefacts

For assessed work you will receive a **time-limited challenge JSON** from staff. Your submission must contain evidence that you actually ran the lab tasks rather than producing a purely text-based answer.

You must submit:
- `artifacts/anti_ai/week11_challenge.json` (provided by staff)
- `pcap/week11_capture.pcap` (captured by you)
- `artifacts/anti_ai/week11_evidence.json` (generated by you)

Recommended workflow:

```bash
# 1) Start your services (Docker Nginx stack or the Python load balancer + backends)

# 2) Capture traffic (HTTP + DNS)
./scripts/capture_traffic.py --filter "tcp port 8080 or udp port 53" -o pcap/week11_capture.pcap

# 3) In another terminal, generate evidence (sends tokenised HTTP requests and a DNS query)
python scripts/anti_ai_generate_evidence.py   --challenge artifacts/anti_ai/week11_challenge.json   --pcap pcap/week11_capture.pcap   --lb-url http://localhost:8080/

# 4) Self-check before submitting
python scripts/anti_ai_validate.py   --challenge artifacts/anti_ai/week11_challenge.json   --evidence artifacts/anti_ai/week11_evidence.json   --pcap pcap/week11_capture.pcap
```

What the validator checks:
- the challenge is within its TTL
- the challenge token appears in the capture as an `X-AI-Challenge` HTTP header
- at least two distinct backends were observed (`X-Served-By` or `X-Backend-ID`)
- a DNS query name containing the token (for example `ai-<token>.invalid`) appears in the capture
