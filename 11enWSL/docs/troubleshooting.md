...
7. [Prediction Verification](#7-prediction-verification)

---

## 7. Prediction Verification

When your observations don't match predictions, use this diagnostic approach.

### 7.1 Load Distribution Unexpected

**Prediction:** "Round-robin should give 3-3-3 distribution"
**Reality:** You see 4-3-2 or similar

**Verification steps:**
1. Check all backends are running: `docker ps`
2. Verify no backend failures occurred during test
3. Increase sample size (send 30+ requests, not 9)
4. Check for keep-alive connections affecting distribution

See `docs/misconceptions.md` section 1.1 for explanation.

### 7.2 Backend Failure Not Detected

**Prediction:** "Stopping backend should immediately remove it from rotation"
**Reality:** First request to dead backend still fails

**Verification steps:**
1. Nginx uses passive health checks by default
2. The first request to a dead backend returns 502
3. After `max_fails` (default 1), backend is marked down

See `docs/misconceptions.md` section 1.3 for explanation.

### 7.3 DNS TTL Not Respected

**Prediction:** "After TTL expires, I should get fresh record"
**Reality:** Old record persists longer than TTL

**Verification steps:**
1. ISP resolvers may enforce minimum TTL
2. Browser caches ignore TTL
3. Use `dig` directly to test authoritative behaviour

See `docs/misconceptions.md` section 2.1 for explanation.
...
**Symptoms:**
```
'tshark' is not recognised as an internal or external command
```
