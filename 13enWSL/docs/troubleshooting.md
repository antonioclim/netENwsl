...
## 🎯 Misconceptions vs Real Issues

Before troubleshooting, verify you're not falling into a common misconception:

| Symptom | Misconception | Reality |
|---------|--------------|---------|
| "Port shows filtered" | Port is protected/secure | Firewall is dropping packets — state is unknown |
| "TLS connection works so it's secure" | TLS = complete security | TLS encrypts content but metadata is visible |
| "QoS 2 but messages lost" | QoS 2 guarantees delivery | QoS is per-hop; check subscriber QoS too |
| "Scan shows open port but can't connect" | Scanner is wrong | Service may have crashed after scan |
| "Anonymous MQTT works on internal network" | Internal = safe | Internal networks can be compromised |

> 📖 For more details, see `docs/misconceptions.md`
