...
1. Navigate to the exercises directory and open the endianness demonstration:
   ```bash
   cd src/exercises
   python3 ex_9_01_demonstrate_endianness.py
   ```
...
3. Connect to the FTP server using the demonstration client:
   ```bash
   cd src/exercises
   python3 ex_9_03_ftp_client_demo.py --host localhost --port 2121 --user test --password 12345
   ```
...
1. Examine the pseudo-FTP server implementation:
   ...
   cat ex_9_02_implement_pseudo_ftp.py
   ```

2. Start the pseudo-FTP server in one terminal:
   ```bash
   python3 ex_9_02_implement_pseudo_ftp.py --mode server --port 60100
   ```

3. In a second terminal, connect with the client:
   ```bash
   python3 ex_9_02_implement_pseudo_ftp.py --mode client --host localhost --port 60100
   ```
...
Five multiple-choice questions targeting common misconceptions about Session and Presentation layers. Use during seminars for interactive learning.

**Location:** `docs/peer_instruction.md`
...
Structured collaborative exercises for working in pairs on this week's laboratory.

**Location:** `docs/pair_programming_guide.md`

**Exercises:**
- P1: Binary Protocol Header (25 min)
- P2: FTP Session Lifecycle (30 min)
- P3: Pseudo-FTP Implementation (35 min)

### Common Misconceptions

Eight documented misconceptions with corrections and verification commands.

**Location:** `docs/misconceptions.md`

**Key misconceptions:**
- "TCP connection = session" (wrong!)
- "FTP uses one connection" (it uses two!)
- "x86 uses big-endian" (it uses little-endian!)
- "CRC-32 prevents tampering" (only detects accidents!)

### Glossary

Technical terms and definitions for Week 9 concepts.

**Location:** `docs/glossary.md`

### Code Tracing Exercises

Mental execution exercises for struct.pack/unpack operations.

**Location:** `docs/code_tracing.md`

### Parsons Problems

Code reordering exercises to practice protocol implementation patterns.

**Location:** `docs/parsons_problems.md`
