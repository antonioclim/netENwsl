...
def check_docker() -> bool:
    """Check whether Docker is available and the daemon responds."""
    ...
    try:
        ...
        # Common failure modes: daemon not running, permission denied, not installed.
        detail = (result.stderr or result.stdout or "").strip()
        if detail:
            print(f"FAIL ({detail.splitlines()[0]})")
        else:
            print("FAIL")
        return False
...
def check_exercises() -> bool:
    """Check whether exercise files are valid Python."""
...
def check_endianness_demo() -> bool:
    """Run the endianness demo as a quick test."""
    print("Running endianness demo...", end=" ", flush=True)

    exercise = PROJECT_ROOT / "src" / "exercises" / "ex_9_01_demonstrate_endianness.py"
