...
> 💡 **Tip:** Many issues stem from conceptual misunderstandings. See [Common Misconceptions](misconceptions.md) to check if your mental model is correct.
...
**Symptoms:**
```
'wsl' is not recognised as an internal or external command
```
...
Use this flowchart to systematically diagnose common issues:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    WEEK 1 TROUBLESHOOTING DECISION TREE                      │
└─────────────────────────────────────────────────────────────────────────────┘
...
START: What's the problem?
         ...
         │         │                   │
         │         │               NO ─┴─► sudo service docker start
         │         │               YES ─► Check socket: ls -la /var/run/docker.sock
         │         │
         │         └─► "Permission denied"
         │                   │
         ...
         │                         └─► If missing: docker compose build
         │
         ...
         │         │         │
         │         │     YES ─┴─► Use localhost or service name
         │         │     NO  ─┴─► Use localhost:HOST_PORT
         │         │
         │         └─► Firewall blocking?
         │                   │
         ...
         │                   │
         │                   └─► If empty: echo "nameserver 8.8.8.8" | sudo tee /etc/resolv.conf
         │
         ├─► "Capture shows no packets"
         │         │
         │         ├─► Correct interface?
         │         │         │
         │         │         └─► For Docker: vEthernet (WSL) or eth0
         │         │             For loopback: lo
         │         │
         ...
         │                   │
         │                   └─► Try: tcpdump -i any (capture all interfaces)
         │
         └─► "Python script error"
                   │
                   ├─► ModuleNotFoundError?
...
When something doesn't work, run these commands in order:
