...
VLSM permits different subnet masks within the same address space, allowing efficient address allocation for networks with different size requirements. This technique reduces address waste by matching subnet sizes to actual host requirements.
...
Address utilisation efficiency measures the percentage of allocated addresses actually required. VLSM typically achieves higher efficiency than FLSM because it avoids allocating excess addresses to small networks. Best VLSM allocation considers both current requirements and potential future growth.
...
Standard practice allocates a /48 prefix to organisations, permitting 65,536 /64 subnets. The /64 subnet size is recommended for all end-user networks because it allows Stateless Address Autoconfiguration (SLAAC) and maintains compatibility with various IPv6 features.
...
When working with IP addressing, watch out for these frequent errors:

1. **Forgetting network and broadcast addresses** — Always subtract 2 from total addresses to get usable hosts
2. **Assuming boundaries fall on octets** — A /20 splits within the third octet, not at .0
3. **Processing VLSM in wrong order** — Always allocate largest requirements first
4. **Using :: multiple times in IPv6** — The :: shorthand can appear only once per address
5. **Confusing /31 with zero hosts** — RFC 3021 allows /31 for point-to-point links

See `docs/misconceptions.md` for detailed explanations of these and other common errors.
