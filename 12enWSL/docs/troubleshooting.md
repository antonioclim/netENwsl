...
2. Wait for the whale icon in the system tray to stabilise
3. Verify with: `docker info`

**WSL2 Integration:**
If Docker commands work in PowerShell but not in WSL:
1. Open Docker Desktop > Settings > Resources > WSL Integration
2. Enable integration with your Linux distribution
3. Restart your WSL terminal: `wsl --shutdown` then reopen
...
3. **Wrong host/port:**
   - Ensure you're connecting to `localhost:1025` not `localhost:25`
...
**Solution:**
Ensure the volume is correctly mounted in `docker-compose.yml`:
```yaml
volumes:
...
1. **Select correct interface:**
   - For Docker: Use `\\.\pipe\docker_engine` or `Npcap Loopback Adapter`
   - For WSL: Use the virtual ethernet adapter
