...
**What to expect:** After `start_lab.py`, you should see 4 services running (SMTP, JSON-RPC, XML-RPC, gRPC).
...
**What to expect:** On connection, you will receive a `220` greeting message.
...
**What to expect:** Each command returns a response code. DATA returns `354`, not `250`.

### SMTP Response Codes

| Code | Meaning | When you'll see it |
|------|---------|-------------------|
| 220 | Service ready | On initial connection |
| 250 | OK, command completed | After EHLO, MAIL FROM, RCPT TO |
| 354 | Start mail input | After DATA command |
| 221 | Closing connection | After QUIT |
| 421 | Service not available | Server overload |
| 450 | Mailbox unavailable | Temporary issue |
| 500 | Syntax error | Malformed command |
| 501 | Syntax error in parameters | Bad arguments |
| 503 | Bad sequence of commands | Wrong order |
...
**What to expect:** `{"jsonrpc":"2.0","result":8.0,"id":1}`
...
**What to expect:** `{"jsonrpc":"2.0","result":42.0,"id":2}`
...
**What to expect:** Array of 3 response objects.
...
**What to expect:** HTTP 204 No Content (empty response body).
...
**What to expect:** HTTP 200 with error object containing code `-32601`.
...
| `echo` | `{message: str}` | The message |
| `sort_list` | `{items: list}` | Sorted list |
| `get_time` | None | Server time |
| `get_server_info` | None | Server details |
| `get_stats` | None | Call statistics |
...
**What to expect:** XML response containing `<int>8</int>`.
...
**What to expect:** XML array of method names.
...
**What to expect:** JSON with `{"result": 8}`.
...
**What to expect:** Table comparing latency and throughput across protocols.
...
# Run specific test
pytest tests/test_exercises.py::TestExercise1SMTP

# Run tests for exercise 1 only
pytest tests/test_exercises.py -e 1
...
|---------|------|----------|--------------|
| SMTP | 1025 | TCP | `nc localhost 1025` |
| JSON-RPC | 6200 | HTTP/JSON | `curl -X POST http://localhost:6200 ...` |
| XML-RPC | 6201 | HTTP/XML | `curl -X POST http://localhost:6201 ...` |
| gRPC | 6251 | HTTP/2 | `grpcurl -plaintext localhost:6251 list` |
| Portainer | 9000 | HTTP | Browser: http://localhost:9000 |
