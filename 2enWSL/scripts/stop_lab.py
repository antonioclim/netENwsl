...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Stop Week 2 Laboratory Environment (WSL2)",
...
Notes:
  ...
  - Portainer is a global service that should remain running
    ...
    logger.info("=" * 60)
    logger.info("Stopping Week 2 Laboratory Environment")
    ...
    try:
        ...
        try:
            if args.force:
                subprocess.run(["docker", "kill", "week2_lab"], capture_output=True)
            else:
                subprocess.run(["docker", "stop", "-t", str(args.timeout), "week2_lab"],
                             capture_output=True)
            logger.info("  Stopped week2_lab")
        except Exception:
            pass
        return 0
    ...
    try:
        ...
        if running:
            ...
            logger.info("  All week2 containers stopped")
