...
def run_in_container(container: str, command: str, capture: bool = True) -> subprocess.CompletedProcess:
...
def demo_broadcast() -> bool:
    ...
    send_result = run_in_container(
        "week3_client",
        "python3 /app/src/exercises/ex_3_01_udp_broadcast.py send "
        "--dst 255.255.255.255 --port 5007 --count 3 --interval 0.5"
    )
...
def demo_multicast() -> bool:
    ...
    recv_proc = subprocess.Popen([
        ...
        "recv", "--group", multicast_group, "--port", port, "--count", "3", "--timeout", "10"
    ], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    ...
    run_in_container(
        ...
        f"--group {multicast_group} --port {port} --count 3 --interval 0.5 --ttl 4"
    )
