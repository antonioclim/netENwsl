...
def demo_docker_nginx() -> bool:
    ...
    for i in range(12):
        ...
        if status == 200:
            ...
            backend: str = headers.get("x-backend-id", "unknown")
            backend_name: str = headers.get("x-backend-name", "Unknown")
...
def demo_load_balance() -> bool:
    ...
    for algo_name, url in algorithms.items():
        ...
        for i in range(6):
            ...
            if status == 200:
                backend: str = headers.get("x-backend-id", "?")
...
def main() -> int:
    ...
    # Default: show menu
    list_demos()
    print("Usage: python scripts/run_demo.py --demo <n>")
    return 0
